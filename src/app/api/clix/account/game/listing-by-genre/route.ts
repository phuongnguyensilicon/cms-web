...
const getMediaByGameGenre = async (req: Request, res: NextApiResponse) => {
  ...
  const results = await gameService.getMediaItemsByGameGenreId(token, requestData.id);
  if (!results) {
    return throwResponseError(ERROR_MESSAGE.COMMON_ERROR, StatusCodeEnum.BAD_REQUEST);
  }
...
export { getMediaByGameGenre as GET };